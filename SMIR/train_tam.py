...
class SpectrogramFingerprintData(Data.Dataset):
    ...
    def __getitem__(self, index):       
        ...
        if int(file.split("-")[1].split(".")[0]) == 0:
            start_index = 0
            chunk_len = min(len(data), 32)
            is_anchor = 1
        else:
            start_index, chunk_len = generate_chunk(min(len(data), 32))

        margin = 0.2 * chunk_len / len(data) + 0.2
        # margin = 0.4
        sequence = np.zeros((32, 128))
...
def train():
    ...
    for step, (tensor, label, margin, is_anchor, padding_mask) in enumerate(pbar_train):
        tensor = tensor.cuda()
        label = label.cuda()
        margin = margin.cuda()
        is_anchor = is_anchor.cuda()
        padding_mask = padding_mask.cuda()
        
        local_embedding = model(tensor, padding_mask)
        logit, loss = tam_loss(local_embedding, label, margin, is_anchor)   
...
def init_class_center_128():
    ...
    with torch.no_grad():
        tam_loss.weight.data = t_c


def parse_opt():
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset_path", type=str, default='./datasets/mt/train/data-10w')
    ...
    parser.add_argument("--class_nums", type=int, default=100000)
...
if __name__ == "__main__":
    ...
    logger = get_logger()
    log_dir = f"./runs/checkpoint/{opt.model}_tam/exp/{opt.model}.pth"
    ...
    tam_loss = TamLoss(in_features=opt.feature_dim, out_features=opt.class_nums, s=32.0).cuda()
    optimizer = torch.optim.Adam([{'params': model.parameters()}, {'params': tam_loss.parameters()}], lr=opt.lr, weight_decay=opt.weight_decay)
    scheduler = CosineAnnealingLR(optimizer, T_max=opt.epoch * len(train_data), eta_min=opt.last_lr)

    if os.path.exists(log_dir):
        checkpoint = torch.load(log_dir)
        model.load_state_dict(checkpoint["model"])
        tam_loss.load_state_dict(checkpoint["tam_loss"])
        ...
        df_train.to_csv(f"./runs/checkpoint/{opt.model}_tam/exp/result.csv", index=False)
        start_epoch = 0

    for epoch in range(start_epoch + 1, opt.epoch + 1):
        if epoch == 1:
            init_class_center_128()

        train_loss, train_acc = train()
        time_now = time.strftime("%Y-%m-%d-%H.%M", time.localtime())
        data_list = [time_now, epoch, train_loss,train_acc]
        data = pd.DataFrame([data_list])
        data.to_csv(f"./runs/checkpoint/{opt.model}_tam/exp/result.csv", mode="a", header=False, index=False)

        state = {
            "model": model.state_dict(),
            "tam_loss": tam_loss.state_dict(),
        ...
        if epoch % 5 == 0:
            ...
            with open(f"./runs/checkpoint/{opt.model}_tam/exp/table.txt", "a") as file:
                file.write(f"epoch: {epoch}\n")
                file.write(table_string + "\n")

            torch.save(state, f"./runs/checkpoint/{opt.model}_tam/exp/{opt.model}_{epoch}.pth".format(epoch)) 
